...
# .sort_values( ascending=True)
        ...
        st.write("you have "+str(int(xlen/16)) +" solutions of "+str(int(xlen)))
        # print(totalData)
        ...
        st.dataframe(solutions[['sol_index','Water','Production','city skip','Total','value']])
        ...
        fig = make_subplots(specs=[[{"secondary_y": True}]])
        fig.add_trace(go.Bar(x=original['Crop_Name'],
                     y=original["Water"],name =
                     'Original',
                     text=original["Water"])) 
        ...
        fig = make_subplots(specs=[[{"secondary_y": True}]])
        fig.add_trace(go.Bar(x=original['Crop_Name'],
                     y=original["Production"],name =
                     'Original',
                     text=original["Production"])) 
        ...
        fig.update_layout(
            title_text='Production Defrance'
            )
        st.plotly_chart(fig)
...
# =============================summer==============================
        ...
        xlen = len(merged_table1.loc[merged_table1['value']==select_W_S])
        st.write("you have "+str(int(xlen/13)) +" solutions of "+str(int(xlen)))
        # print(totalData)
        solutions = merged_table2.loc[merged_table2['value']==select_W_S]
